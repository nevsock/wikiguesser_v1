...
function App() {
  ...
  const getGenreDescription = () => {
    if (isAllGenreSelected) return '複数ジャンルをチェックすると、そのカテゴリから抽選されます。';
    return getActiveGenreOptions()
      .map((option) => option.description || `${option.label}カテゴリから抽選`)
      .join(' / ');
  };
...
const fetchGenreTitle = async (genreOption) => {
  ...
  const handleGuess = (event) => {
    ...
    setGuessHistory((prev) => [{ word: guess, matches: matchedCount }, ...prev]);
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-800 font-sans selection:bg-indigo-200">
      <div className="max-w-3xl mx-auto p-4 pb-28">
        ...
        <section className="mb-6 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
          <div className="flex flex-col gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-wide text-slate-400">ジャンル設定</p>
              <p className="text-base font-semibold text-slate-800">{getGenreSummary()}</p>
              <p className="text-xs text-slate-500 mt-1">{getGenreDescription()}</p>
            </div>
        ...
        >
          <div className="max-w-3xl mx-auto flex gap-2">
            <input
              ...
              placeholder="記事タイトルを予想して入力..."
              className="flex-1 px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl focus:outline-none focus:border-indigo-500 focus:bg-white transition font-medium"
              autoFocus
            />
            <button
              type="submit"
